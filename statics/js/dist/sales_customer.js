...
function initEvent() {
	...
	$("#btn-export").click(function(e) {
		...
		if (Business.verifyRight("PUREPORTPUR_EXPORT")) {
			...
			Business.getFile(report_sales_customer_xls, t)
		}
	});
	$(".grid-wrap").on("click", ".link", function(e) {
		...
		if (Business.verifyRight("PUREOORTDETAIL_QUERY")) {
			...
			parent.tab.addTabItem({
				...
				text: "销售明细表",
				url: report_sales_detail+"?action=detail&beginDate=" + filterConditions.beginDate + "&endDate=" + filterConditions.endDate + "&customerNo=" + t + "&goodsNo=" + i + "&storageNo=" + a
			});
...
function reloadReport() {
	...
	window.location = report_sales_customer+"?action=supply" + e
}
