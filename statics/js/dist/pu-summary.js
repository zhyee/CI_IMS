...
function initEvent() {
	...
	$("#btn-export").click(function(e) {
		...
		if (Business.verifyRight("PUREPORTINV_EXPORT")) {
			...
			Business.getFile(report_invpu_summary_xls, t)
		}
	});
	$(".grid-wrap").on("click", ".link", function(e) {
		...
		if (Business.verifyRight("PUREOORTDETAIL_QUERY")) {
			...
			parent.tab.addTabItem({
				...
				text: "采购明细表",
				url: report_invpu_detail+"?beginDate=" + filterConditions.beginDate + "&endDate=" + filterConditions.endDate + "&customerNo=" + filterConditions.customerNo + "&goodsNo=" + t + "&storageNo=" + i
...
function reloadReport() {
	...
	window.location = report_invpu_summary+"?action=inv" + e
